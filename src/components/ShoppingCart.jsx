...
const ShoppingCart = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      {cartItems.length !== 0 && (
        <div
          className={`lg:w-[400px] w-[300px] h-screen bg-gray-200 fixed top-0 z-30 border-l-4 border-red-200 rounded-tl-lg ${
            isOpen ? 'right-0' : 'lg:-right-[400px] -right-[300px]'
          }`}
        >
          ...
          <div className="h-full flex flex-col gap-y-3 overflow-y-scroll px-5 pb-24 pt-20">
            {cartItems?.map((item) => {
